"""
Cliente das vagas publicadas como issues no GitHub (ex: frontendbr/vagas): busca na API
pública, extrai o e-mail de candidatura do corpo e monta o e-mail. A orquestração (pedido
de aprovação, envio depois do clique) fica em source.py (GitHubSource).
...
def check_new_issues(config: dict) -> list[dict]:
    ...
    quando o corpo não tem e-mail — quem chama (GitHubSource.run_cycle) decide o que fazer.
